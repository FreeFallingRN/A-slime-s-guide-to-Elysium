...
To wrap this project into standalone native packages for Google Play or iOS App Store, you can use **Capacitor**:

### 1. Initialize Capacitor
Run this in your workspace directory (using the Docker container context):
```bash
# Install Capacitor CLI
docker compose exec app npm install @capacitor/core @capacitor/cli

# Initialize the project config
docker compose exec app npx cap init "A Slime's Guide to Elysium" "com.arsondevs.slimeelysium" --web-dir=dist
```
...
### 3. Sync Assets
Every time you compile the Svelte build, sync the static assets to the native folders:
```bash
# Build Svelte files
...
```
* Once sync completes, open the `/android` folder in **Android Studio** or `/ios` in **Xcode** to compile your `.apk` or `.ipa` app packages!
