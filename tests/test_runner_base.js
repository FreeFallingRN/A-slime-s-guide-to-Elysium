  ...
  if (customAbilities) {
    ...
    if (!Array.isArray(customAbilities) && typeof customAbilities === "object") {
      return Object.entries(customAbilities).map(([id, level]) => {
        const baseAb = characterData.abilities.find((a) => a.id === id) || {
          id,
          value: 0.1,
          target: "digestion"
        };
  ...
  function addTest(chapterOrObj, expected, combat = false, description = "", stage = "final") {
    ...
    if (typeof chapterOrObj === "object" && chapterOrObj !== null) {
      ...
      cases.push({
        ...
        expected:
          obj.expected !== undefined ? obj.expected : obj.val !== undefined ? obj.val : obj.target,
  ...
  function run(title) {
    ...
    cases.forEach(
      ({
        ...
        const lvl =
          playerLvl !== undefined ? playerLvl : chapter !== undefined ? getHalonLvl(chapter) : 1;
